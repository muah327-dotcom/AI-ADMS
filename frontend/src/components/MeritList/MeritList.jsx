...
const MeritList = ({ admin = false }) => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
        ...
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-gray-900 flex items-center gap-3">
            ...
            {programDetails?.current_merit_list && (
              <span className="text-sm bg-primary-100 text-primary-800 border border-primary-200 px-3 py-1 rounded-full font-mono">
                Merit List #{programDetails.current_merit_list}
              </span>
            )}
          </h1>
          <p className="text-gray-500 mt-1">View program-wise merit rankings, fee statuses, and admissions</p>
        </div>
        {admin && (
          <div className="flex flex-wrap gap-3">
            <button
              onClick={() => setShowFeeConfig(!showFeeConfig)}
              className="inline-flex items-center px-4 py-2 bg-white border border-gray-200 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors shadow-sm"
            >
      ...
      {admin && showFeeConfig && (
        <div className="bg-white rounded-xl border border-primary-500/30 p-6 space-y-4 animate-fade-in">
          ...
          <form onSubmit={handleSaveFeeConfig} className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-xs text-gray-500 mb-1">Admission Fee (PKR)</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1">Tuition Fee (PKR)</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1">Fee Payment Deadline</label>
              <input
                type="date"
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="flex-1">
            <label className="text-sm font-medium text-gray-500 mb-1 block">Select Program</label>
            <select
          ...
          {admin && (
            <div>
              <label className="text-sm font-medium text-gray-500 mb-1 block">Category Filter</label>
              <select
                className="w-full px-4 py-2 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none text-gray-900"
      ...
      {/* Program Info */}
      {selectedProgramData && (
        <div className="bg-primary-50 rounded-xl p-6 border border-primary-500/20 shadow-sm">
          <div className="flex justify-between items-start">
            <div>
              <h3 className="text-lg font-semibold text-gray-900">{selectedProgramData.name}</h3>
              <p className="text-gray-500">{selectedProgramData.department}</p>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 mt-4">
            ...
            <div className="bg-white rounded-lg p-3 text-center border border-gray-200 shadow-sm">
              <p className="text-2xl font-bold text-primary-600">{stats.selected}</p>
              <p className="text-xs text-gray-500">Selected (Pending Fee)</p>
            </div>
            <div className="bg-white rounded-lg p-3 text-center border border-gray-200 shadow-sm">
            ...
            <div className="bg-white rounded-lg p-3 text-center border border-gray-200 shadow-sm">
              ...
              <p className="text-xs text-gray-500">Dropped (Unpaid)</p>
            </div>
          </div>
        </div>
      )}
                ...
                {meritList.map((entry) => (
                  <tr key={entry.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center justify-center w-8 h-8 rounded-full font-bold ${entry.rank <= 3 ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-200 text-gray-700'
                        }`}>
                        {entry.rank}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center">
                        <div className="h-8 w-8 rounded-full bg-primary-50 flex items-center justify-center mr-3 border border-primary-500/20">
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${entry.category === 'merit' ? 'bg-yellow-100 text-yellow-800' :
                          entry.category === 'quota' ? 'bg-primary-100 text-primary-800' :
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center">
                        <div className="flex-1 w-20 h-2 bg-gray-200 rounded-full overflow-hidden mr-3">
                          <div
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${entry.status === 'confirmed'
                          ? 'bg-green-100 text-green-800 border border-green-500/30'
                    ...
                    {/* Fee Payment Column */}
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded text-xs font-medium ${entry.fee_status === 'verified' ? 'bg-green-100 text-green-800' :
                          entry.fee_status === 'submitted' ? 'bg-yellow-100 text-yellow-800' :
                            entry.fee_status === 'rejected' ? 'bg-red-100 text-red-800' :
                        ...
                        <div className="flex items-center space-x-2">
                          {entry.fee_receipt_url && (
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="p-1.5 bg-gray-100 hover:bg-gray-200 text-primary-600 rounded transition-colors"
      ...
      {showPreviousListsModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-xl max-w-md w-full border border-gray-200 shadow-2xl p-6">
                ...
                Array.from({ length: programDetails.current_merit_list }).map((_, i) => (
                  <div key={i} className="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-lg shadow-sm">
                    <div className="flex items-center">
                      ...
                      <div>
                        <p className="font-bold text-gray-900">Merit List #{i + 1}</p>
                        <p className="text-xs text-gray-500">{selectedProgramData?.name}</p>
                      </div>
                    </div>
                    <button
                      onClick={() => exportToCSV(i + 1)}
                      className="p-2 bg-gray-100 hover:bg-gray-200 text-gray-700 hover:text-gray-900 rounded-lg transition-colors border border-gray-200"
                      title="Export CSV"
                    >
